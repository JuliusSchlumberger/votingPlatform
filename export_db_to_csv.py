...
# Query the database
for output in ['peiling', 'stemming']:
    print(output)
    query = f"SELECT * FROM {output}_responses"
    ...
    GITHUB_API_URL = f'https://api.github.com/repos/{GITHUB_REPO}/contents/{output}_responses.csv'
    message = f"Add {output} responses CSV"
    ...
    if response.status_code in [200, 201]:
        print(f"Data exported to {output}_responses.csv and uploaded to GitHub")
    else:
